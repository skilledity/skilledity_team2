...
export const studentLogin = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: student.id, name: student.name, email: student.email }, // Payload with student data
    ...
    res.cookie("auth_token", token, {
      httpOnly: true, // This cookie is accessible only by the server
      maxAge: 15 * 60 * 1000, // Cookie expiry (15 minutes)
...
export const logoutStudent = async (req, res) => {
  try {
      ...
      return res.status(200).json({ message: 'Logged out successfully' }); // Explicitly returning status code
  } catch (error) {
      console.error('Error during student logout:', error); // Log error for debugging
      return res.status(500).json({ error: 'Internal Server Error' });
  }
};
